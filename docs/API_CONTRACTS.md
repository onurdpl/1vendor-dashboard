...
- Threshold-aged `orders/create` events that remain `PROCESSING` are evaluated request-by-request and projected as `processing_review_required` items. An unexpired lease is excluded even when `receivedAt` is old; expired and legacy no-lease work remains supervised after the 15-minute threshold. The review is diagnostic-only and never authorizes replay, retry, reset, event/job mutation, or repair execution.
- Processing-review items expose only safe evidence: deterministic signal and webhook-event IDs, Shopify webhook ID, safe order number/ID, received time/age, retained-payload availability, processing generation, attempt/max-attempt counts, lease expiry/state, executor-enabled state, latest linked job status/timestamps/retry count, local commerce classification, allocation count, and sale-ledger count. `OperationalJob` suppression authority is always false. Raw payload and customer data are not returned.
- Current-State Repair dry-run may return `summary.executionBlocked=true` with `summary.executionBlockedReason=active_shopify_order_intake`. Execute rechecks this condition and local plan assumptions inside the shared order-locked transaction and returns `409` when a fresh dry-run is required.
...
- Customer cancellation is full-order only. The customer cannot select items, quantities, vendors, or allocations.
- `GET /api/customer-cancellations/eligibility?shopifyOrderId=<Shopify Order GID or ID>` returns `canRequestCancellation=true` only when every canonical order line at its full ordered quantity is safe in the same request. One unsafe line or allocation makes the complete order unavailable; there is no partial fallback.
- `POST /api/customer-cancellations/requests` accepts only `shopifyOrderId`, `reasonCode`, optional `note`, and `idempotencyKey`. Supplying `items` is rejected. After canonical customer ownership is verified, the backend derives every required line, full quantity, and exact local allocation.
- `GET /api/customer-cancellations/status?shopifyOrderId=<Shopify Order GID or ID>` remains available regardless of the intake flag and returns customer-safe order-level parent request status and timestamps for the canonically owned order. Internal child outcomes are not exposed as customer partial-cancellation results.
...
- `SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID` is the Customer Account app Client ID and JWT audience. `SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_SECRET` is used only to verify Customer Account HS256 session tokens. It does not fall back to the webhook-HMAC-only `SHOPIFY_WEBHOOK_SECRET`, so the Customer Account routes fail closed when the dedicated authority is absent. Enabling intake requires the dedicated Client ID, Client Secret, and canonical shop domain; disabled intake remains production-startup compatible before a dedicated cancellation app is configured.
...
- Successful creation persists through the canonical-order lock and database idempotency boundary. The transaction revalidates the complete local order scope and persists either the parent plus every required child at full quantity or nothing. Every affected allocation receives the existing shipment and finance holds.
...
- When `CUSTOMER_CANCELLATION_AUTO_REFUND_ENABLED=true`, durable child-scoped `REFUND_SYNC` jobs process the internally derived full-order scope. Sibling children are serialized with the existing order lock and active-attempt evidence: an active sibling defers/retries instead of becoming a terminal conflict, and the next child re-fetches canonical refund authority before mutation. The flag defaults to `false`.
...
- The parent request becomes `APPROVED` only when every required internal child is authoritatively `APPROVED`. `PARTIALLY_RESOLVED` means heterogeneous internal outcomes still require exception resolution; it never means the customer successfully cancelled only part of an order.
- Clean pre-shipment customer cancellations are automatic work and do not require routine Admin approval. `GET /admin/operations?type=customer_cancellation_exception` is an admin-only, paginated, visibility-only queue derived from persisted terminal/conflict evidence; ordinary clean `PENDING` work and completed `APPROVED`/`DECLINED` history are excluded. The Admin order breakdown includes item-scoped cancellation request, hold, job, attempt, and exception evidence but exposes no retry/refund/resolve mutation.
