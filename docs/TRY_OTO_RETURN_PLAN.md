...
- The forward `deliveryOptionId` must be persisted at the delivery-option-selected stage and copied into async recovery snapshots before `Try OTO shipment pending provider finalization` is stored. Earlier async recovery snapshots only kept a `deliveryOptionIdPresent` boolean, so later return creation could not recover the actual documented id.
- Webhook and status refresh updates must preserve the stored forward delivery option metadata. Admin diagnostics should show whether it was retained after webhook or refresh updates.
...
- Return probes require `returnOrderId`. If return creation is blocked before provider call, probe actions stay disabled and the UI should explain that no return probe can run until a return order exists.
- Admin diagnostics should show whether the forward delivery option was available, its source, its persistence stage, whether return `deliveryOptionId` was present, its source, and whether `pickupLocationCode`, `sku`, and `quantity` were present.
