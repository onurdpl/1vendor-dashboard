...
A persisted pending customer-cancellation item is a non-monetary, allocation-scoped finance hold identified by `CUSTOMER_CANCELLATION_PENDING`. `APPROVED_FOR_REFUND` retains the same hold because it means refund authorization exists (from Admin review or the default-disabled clean pre-shipment processor); Shopify refund execution and reconciliation are still pending. Customer intake is full-order only, so an accepted request creates internal child holds for every affected allocation or creates nothing. Each affected allocation is excluded from new settlement drafts, settlement approval, payout preparation and payout transitions; economic transfer is also blocked. Final child `APPROVED` requires verified monetary refund evidence for the exact derived line-item quantity, and the parent becomes `APPROVED` only when every required child succeeds. Final approval and payout transitions revalidate persisted request state so a hold created after an earlier preview still wins.
