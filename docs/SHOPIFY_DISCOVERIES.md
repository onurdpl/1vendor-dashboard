...
- `APPROVED_FOR_REFUND` is an intermediate workflow state: Admin authorization or default-disabled clean pre-shipment auto-authorization may exist, but Shopify refund execution and canonical reconciliation have not completed. It remains active hold authority for the affected item/allocation. `REFUNDED_AWAITING_ORDER_CANCEL` means exact product refund quantity has been canonically verified and no checkout-shipping refund was requested by the customer-cancellation flow, but Shopify Order cancellation has not yet been canonically confirmed. It remains active shipment and finance hold authority. Final `APPROVED` now requires the future Shopify order-cancellation confirmation step and releases this customer-cancellation hold; `DECLINED`, `TOO_LATE`, and `CONFLICTED` retain their existing no-hold semantics.
...
- `CUSTOMER_CANCELLATION_AUTO_REFUND_ENABLED` defaults to `false`. When explicitly enabled, the durable item-scoped processor revalidates clean pre-shipment authority, transitions `PENDING` to `APPROVED_FOR_REFUND` without a fake Admin actor, and submits an exact-quantity product-only Shopify refund with no checkout-shipping refund and `notify=false`. Refund submission alone does not release holds; verified `refunds/create` or canonical monetary evidence must reconcile the exact quantity to `REFUNDED_AWAITING_ORDER_CANCEL`. That state is the idempotent starting point for the future Shopify `orderCancel` step and prevents another refund submission. Exceptions remain held for later Admin Review. Vendor Reject behavior, native Shopify cancellation settings, and the not-yet-live Customer Account UI extension are unchanged.
