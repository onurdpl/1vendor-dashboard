...
- The same persisted pending authority now creates a separate, non-monetary allocation finance hold. It blocks new settlement, payout, and economic-transfer progression with `CUSTOMER_CANCELLATION_PENDING` but does not create a refund, reverse a sale, create vendor debt, pay anything, or mutate Shopify. Admin resolution and refund behavior remain outside this phase.
- `APPROVED_FOR_REFUND` is an intermediate workflow state: Admin authorization or default-disabled clean pre-shipment auto-authorization may exist, but Shopify refund execution and canonical reconciliation have not completed. It remains active hold authority for the affected item/allocation. For full-order pre-fulfillment customer cancellations, Shopify `suggestedRefund.shipping.maximumRefundableSet` is the customer-paid checkout-shipping refund authority; positive refundable shipping is included through `RefundInput.shipping.amount`, while zero/free/fully-discounted shipping records an intended `0.00` shipping refund and sends no shipping input. `REFUNDED_AWAITING_ORDER_CANCEL` means exact product refund quantity and intended checkout-shipping refund amount have been canonically verified, but Shopify Order cancellation has not yet been canonically confirmed. It remains active shipment and finance hold authority. Final `APPROVED` now requires the future Shopify order-cancellation confirmation step and releases this customer-cancellation hold; `DECLINED`, `TOO_LATE`, and `CONFLICTED` retain their existing no-hold semantics.
...
- The Customer Account UI extension is not live. Customer Account session-token verification uses `SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_ID` as the JWT audience and the dedicated `SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_SECRET` as the HS256 signing authority. It never uses the Admin API token or falls back to `SHOPIFY_WEBHOOK_SECRET`; that variable remains webhook-HMAC-only. The dedicated pair may belong to a separate Customer Cancellation Shopify app. With intake disabled, the dedicated secret may remain absent without blocking backend startup; all Customer Account authentication still fails closed until it is configured. Shopify protected-customer-data access must be sufficient for the signed-in customer `sub`; there is no insecure fallback when `sub` is absent.
- `CUSTOMER_CANCELLATION_AUTO_REFUND_ENABLED` defaults to `false`. When explicitly enabled, the durable item-scoped processor revalidates clean pre-shipment authority, transitions `PENDING` to `APPROVED_FOR_REFUND` without a fake Admin actor, and submits an exact-quantity Shopify refund for all customer-cancelled product lines plus the canonically refundable checkout-shipping amount, if positive, with `notify=false`. Refund submission alone does not release holds; canonical monetary evidence must reconcile the exact product quantity and intended shipping amount to `REFUNDED_AWAITING_ORDER_CANCEL`. That state is the idempotent starting point for the future Shopify `orderCancel` step and prevents another refund submission. Exceptions remain held for later Admin Review. Vendor Reject behavior, native Shopify cancellation settings, and the not-yet-live Customer Account UI extension are unchanged.
- Customer cancellation `orderCancel` uses Admin GraphQL `orderCancel` with `reason=CUSTOMER`, `restock=false`, `refundMethod.originalPaymentMethodsRefund=false`, and `notifyCustomer=false`. It never uses the deprecated `refund` argument and does not issue a second refund or shipping refund. Mutation acceptance is not final authority: the returned Shopify `Job` must complete, and canonical Shopify order `cancelledAt` must be observable before the request/item may transition from `REFUNDED_AWAITING_ORDER_CANCEL` to `APPROVED`.
- Custom customer cancellation intake is full-order only. The public client supplies no line, quantity, vendor, or allocation authority. The backend derives every canonical order line at its full quantity, requires every line/allocation to be safe, and persists all internal child rows or none under the canonical-order lock. A prior partial refund, return overlap, shipment/fulfillment authority, unsafe allocation, progressed finance, ambiguous mapping, or insufficient full-quantity authority makes the complete order unavailable.
- Internal cancellation children remain allocation-scoped hold, refund, reconciliation, and exception records. Sibling refund jobs serialize through the existing order lock and active outbound-attempt evidence; an active sibling is retryable/deferred, and canonical refund authority is refreshed before each subsequent submission. The parent reaches `APPROVED` only after every child is verified `APPROVED` by the future order-cancellation confirmation step; children in `REFUNDED_AWAITING_ORDER_CANCEL` keep the parent non-final. `PARTIALLY_RESOLVED` is an internal exception state, not customer-visible partial cancellation.
- Historical Shopify RequestedEdit evidence, including order `#1124`, is not imported into the local request model.
- Local customer-cancellation preview tooling must use `backend/.env.customer-cancellation-preview` through the supported `backend:customer-cancellation:preview`, `backend:customer-cancellation:preview:check`, and `backend:customer-cancellation:preview:probe` commands. The tooling resolves this env file by absolute repository path, independent of the caller's current working directory, and rejects `DATABASE_URL` before any preview Prisma probe unless the host is `localhost`, `127.0.0.1`, or `::1` and the database name is exactly `vendor_dashboard_customer_cancellation_preview`. Do not run raw Prisma/customer-cancellation preview snippets with manually sourced env files; use the guarded preview commands instead.
