...
- `APPROVED_FOR_REFUND` is an intermediate workflow state: admin authorization may exist, but Shopify refund execution and canonical reconciliation have not completed. It remains active hold authority for the affected item/allocation. Final `APPROVED` remains terminal and releases this customer-cancellation hold; `DECLINED`, `TOO_LATE`, and `CONFLICTED` retain their existing no-hold semantics.
...
- Admin review endpoints/UI, Shopify mutation, refund execution, finance changes, notification, and native Shopify cancellation-setting changes remain unimplemented. The internal lifecycle permits only the guarded `PENDING` to `APPROVED_FOR_REFUND` transition needed by the later Admin Review phase; it cannot mark refund execution complete.
