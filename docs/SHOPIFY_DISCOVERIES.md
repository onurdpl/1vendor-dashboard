...
- `APPROVED_FOR_REFUND` is an intermediate workflow state: Admin authorization or default-disabled clean pre-shipment auto-authorization may exist, but Shopify refund execution and canonical reconciliation have not completed. It remains active hold authority for the affected item/allocation. Final `APPROVED` requires exact line-item quantity plus verified monetary refund evidence and releases this customer-cancellation hold; `DECLINED`, `TOO_LATE`, and `CONFLICTED` retain their existing no-hold semantics.
