...
describe('Paratika payment return route skeleton', () => {
  it('accepts GET returns without mutating payment or Shopify state', async () => {
    ...
    expect(result).toEqual({
      ...
      receivedStatus: '99',
      receivedReference: 'SPORGYM-SHOPIFY-order-100',
      diagnostics: {
        query: {
          keyCount: 4,
          sensitiveKeyCount: 2,
          keys: ['responseCode', 'MERCHANTPAYMENTID', 'cardNumber', 'token'],
          safeValues: {
            responseCode: '99',
            MERCHANTPAYMENTID: 'SPORGYM-SHOPIFY-order-100',
            cardNumber: '[redacted]',
    ...
    expect(logInfo).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        queryKeyCount: 4,
        bodyKeyCount: 0,
        queryKeys: ['responseCode', 'MERCHANTPAYMENTID', 'cardNumber', 'token'],
        querySafeValues: {
          responseCode: '99',
          MERCHANTPAYMENTID: 'SPORGYM-SHOPIFY-order-100',
          cardNumber: '[redacted]',
  ...
  it('accepts POST returns while ignoring unknown params safely', async () => {
      ...
      expect.objectContaining({
        ...
        diagnostics: {
          query: {
            ...
            sensitiveKeyCount: 0,
            keys: ['unknown'],
            safeValues: { unknown: 'ignored' },
          },
          body: {
            keyCount: 3,
            sensitiveKeyCount: 2,
            keys: ['responseCode', 'merchantPassword', 'cvv'],
            safeValues: {
              responseCode: '00',
    ...
    expect(logInfo).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        queryKeyCount: 1,
        bodyKeyCount: 3,
        bodyKeys: ['responseCode', 'merchantPassword', 'cvv'],
        bodySafeValues: {
          responseCode: '00',
          merchantPassword: '[redacted]',
          cvv: '[redacted]',
