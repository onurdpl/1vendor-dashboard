...
export const runtimeServices = {
    ...
    matchVendorLogoIsbasiFirm: (vendorId: string) =>
      runtimeConfig.apiMode === 'real'
        ...
        : Promise.resolve({
            ...
            searchedBy: {
              logoIsbasiCustomerCodePresent: false,
      ...
      if (runtimeConfig.apiMode === 'real') {
        ...
        return {
          ...
          user: createCurrentUserFromVendorAccess({
            ...
            vendorAccess: loginResponse.user.vendorAccess,
          }),
        };
      }
  ...
  orders: {
    ...
    async createShipmentExecution(allocationId: string, vendorId = getCurrentVendorId(), customerOverrides?: ShipmentCustomerOverrides) {
      ...
      const submittedAt = new Date().toISOString();
      return {
        id: `mock-shipment-kargonomi-${allocationId}`,
        allocationId,
        vendorId,
        ...
        providerShipmentId: `mock-kargonomi-${allocationId}`,
        trackingNumber: `KG-${allocationId.slice(-6).toUpperCase()}`,
        trackingUrl: null,
        labelUrl: null,
    ...
    async retryShipmentExecution(shipmentExecutionId: string) {
      ...
      return {
        ...
        sourceShopifyFulfillmentId: null,
        provider: 'kargonomi' as const,
        providerShipmentId: `mock-kargonomi-retry-${shipmentExecutionId.slice(-6).toUpperCase()}`,
        trackingNumber: `KG-${shipmentExecutionId.slice(-6).toUpperCase()}`,
        trackingUrl: null,
        labelUrl: null,
        shipmentStatus: 'created' as const,
        desi: '3.00',
    ...
    async retryFailedShipmentExecution(
      ...
      return {
        ...
        sourceShopifyFulfillmentId: null,
        provider: 'kargonomi' as const,
        providerShipmentId: `mock-kargonomi-recovery-${shipmentExecutionId.slice(-6).toUpperCase()}`,
        trackingNumber: null,
        trackingUrl: null,
        labelUrl: null,
        shipmentStatus: 'created' as const,
    ...
    async refreshShipmentExecutionStatus(shipmentExecutionId: string, vendorId = getCurrentVendorId()) {
      ...
      return {
        id: shipmentExecutionId,
        allocationId: shipmentExecutionId.replace(/^mock-shipment-[^-]+-/, ''),
        vendorId,
        sourceShopifyOrderId: null,
        sourceShopifyOrderNumber: null,
        sourceShopifyFulfillmentId: null,
    ...
    async shippingProviderDiagnostics(
      ...
      return {
        ...
        dummyKargoSupport: 'not_implemented' as const,
        statusSyncSupport: 'not_implemented' as const,
        missing: ['SHIPPING_EXECUTION_ENABLED', 'KARGONOMI_BASE_URL', 'KARGONOMI_API_TOKEN'],
        deprecatedEnvFallbacks: [],
    ...
    async vendorShippingConfig(vendorId = getCurrentVendorId(), options: ReadRequestOptions = {}) {
      ...
      return {
        ...
        warehouses: [
          {
            id: `mock-warehouse-${vendorId}-112668`,
            vendorId,
            provider: 'kargonomi' as const,
            warehouseId: '112668',
    ...
    async updateVendorShippingConfig(vendorId: string, input: VendorShippingConfigUpdate) {
      ...
      return {
        ...
        defaultWarehouseId: input.defaultWarehouseId ?? '112668',
        ...
        warehouses: (input.warehouses ?? []).map((warehouse, index) => ({
          ...
          vendorId,
          provider: warehouse.provider ?? input.preferredProvider ?? 'kargonomi',
