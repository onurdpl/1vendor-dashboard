...
export function AdminRefundAdjustmentsPage() {
  ...
  const [paidCreditPending, setPaidCreditPending] = useState(false);
  ...
  const paidCreditQuery = useQueryResource(
    ['admin', 'finance', 'paid-financial-correction-credit', terminalDetail?.id ?? 'none', terminalDetail?.updatedAt ?? 'none'],
    ({ signal }) => getPaidFinancialCorrectionCreditState(terminalDetail!.id, signal),
    {
      routeName: 'Paid financial correction credit',
      endpoint: terminalDetail ? `/admin/finance/refund-reviews/${terminalDetail.id}/financial-correction-paid-credit` : '/admin/finance/refund-reviews',
      enabled: Boolean(terminalDetail),
    },
  );
  const paidCreditState = terminalDetail && !paidCreditQuery.isFetching && !paidCreditQuery.error
    ? paidCreditQuery.data : null;
  const paidCreditApplication = paidCreditState && paidCreditState.application?.reviewId === terminalDetail?.id
    ? paidCreditState.application : null;
  const beforeSettlementCreditQuery = useQueryResource(
    ['admin', 'finance', 'before-settlement-correction-credit', terminalDetail?.id ?? 'none', terminalDetail?.updatedAt ?? 'none'],
    ({ signal }) => getBeforeSettlementFinancialCorrectionCreditState(terminalDetail!.id, signal),
    {
      routeName: 'Before-settlement correction credit',
      endpoint: terminalDetail ? `/admin/finance/refund-reviews/${terminalDetail.id}/financial-correction-before-settlement-credit` : '/admin/finance/refund-reviews',
      enabled: Boolean(terminalDetail),
    },
  );
  const beforeSettlementCreditState = terminalDetail && !beforeSettlementCreditQuery.isFetching && !beforeSettlementCreditQuery.error
  ...
  const approvedSettlementDeductionQuery = useQueryResource(
    ['admin', 'finance', 'approved-settlement-correction-deduction', terminalDetail?.id ?? 'none', terminalDetail?.updatedAt ?? 'none'],
    ({ signal }) => getApprovedSettlementFinancialCorrectionDeductionState(terminalDetail!.id, signal),
    {
      routeName: 'Approved-settlement correction deduction',
      endpoint: terminalDetail ? `/admin/finance/refund-reviews/${terminalDetail.id}/financial-correction-approved-settlement-deduction` : '/admin/finance/refund-reviews',
      enabled: Boolean(terminalDetail),
    },
  );
  const approvedSettlementDeductionState = terminalDetail && !approvedSettlementDeductionQuery.isFetching && !approvedSettlementDeductionQuery.error
  ...
  const runPaidCredit = async () => {
    if (!eligibleTerminalDetail || !correctionPreview || !paidCreditState?.eligible || paidCreditApplication ||
        paidCreditPending || !currentPaidReason.trim() || currentPaidReason.trim().length > 500 ||
        correctionPreview.economicDirection !== 'VENDOR_CREDIT' ||
        correctionPreview.difference.vendorPayableReversalMinor >= 0) return;
    setPaidCreditPending(true);
    setPaidCreditError(null);
    try {
      await applyPaidFinancialCorrectionCredit(eligibleTerminalDetail.id, {
        previewFingerprint: correctionPreview.previewFingerprint,
        reason: currentPaidReason.trim(),
      });
      setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: '' });
      await Promise.all([paidCreditQuery.refetch(), paidCorrectionQuery.refetch()]);
    } catch (error) {
      setPaidCreditError(error instanceof Error ? error.message : 'Financial correction credit could not be applied.');
      await Promise.all([correctionPreviewQuery.refetch(), paidCreditQuery.refetch(), terminalDetailQuery.refetch()]);
    } finally {
      setPaidCreditPending(false);
    }
  };

  const runBeforeSettlementCredit = async () => {
    if (!eligibleTerminalDetail || !correctionPreview || !beforeSettlementCreditState?.eligible ||
        beforeSettlementCreditApplication || beforeSettlementCreditPending ||
        !currentPaidReason.trim() || currentPaidReason.trim().length > 500 ||
        correctionPreview.economicDirection !== 'VENDOR_CREDIT' ||
        correctionPreview.difference.vendorPayableReversalMinor >= 0) return;
    setBeforeSettlementCreditPending(true);
    setBeforeSettlementCreditError(null);
    try {
      await applyBeforeSettlementFinancialCorrectionCredit(eligibleTerminalDetail.id, {
        previewFingerprint: correctionPreview.previewFingerprint, reason: currentPaidReason.trim(),
      });
      setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: '' });
      await Promise.all([beforeSettlementCreditQuery.refetch(), paidCreditQuery.refetch()]);
    } catch (error) {
      setBeforeSettlementCreditError(error instanceof Error ? error.message : 'Before-settlement credit could not be applied.');
  ...
  const runApprovedSettlementDeduction = async () => {
    if (!eligibleTerminalDetail || !correctionPreview || !approvedSettlementDeductionState?.eligible ||
        approvedSettlementDeductionApplication || approvedSettlementDeductionPending ||
        !currentPaidReason.trim() || currentPaidReason.trim().length > 500 ||
        correctionPreview.economicDirection !== 'VENDOR_DEDUCTION' ||
        correctionPreview.difference.vendorPayableReversalMinor <= 0) return;
    setApprovedSettlementDeductionPending(true);
    setApprovedSettlementDeductionError(null);
    try {
      await applyApprovedSettlementFinancialCorrectionDeduction(eligibleTerminalDetail.id, {
        previewFingerprint: correctionPreview.previewFingerprint, reason: currentPaidReason.trim(),
      });
      setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: '' });
      await Promise.all([approvedSettlementDeductionQuery.refetch(), beforeSettlementDeductionQuery.refetch()]);
    } catch (error) {
      setApprovedSettlementDeductionError(error instanceof Error ? error.message : 'Approved-settlement deduction could not be applied.');
            ...
            <aside className="op-side-panel refund-review-detail-panel" aria-label="Refund evidence review detail panel">
              ...
              {terminalDetail ? (
                <>
                      ...
                      {correctionPreview ? (
                        <>
                          ...
                          {correctionPreview.economicDirection === 'VENDOR_DEDUCTION' &&
                            correctionPreview.difference.vendorPayableReversalMinor > 0 &&
                            approvedSettlementDeductionState?.eligible && !approvedSettlementDeductionApplication ? (
                              <section className="op-panel-section" aria-label="Approved-settlement correction deduction application">
                                <h5>Approved settlement vendor entitlement reduction</h5>
                                <p className="page-description">Historical approved settlement: {formatMinor(approvedSettlementDeductionState.approvedSettlement?.netPayableMinor)}. Financial Correction: -{formatMinor(correctionPreview.difference.vendorPayableReversalMinor)}. Expected first payout entitlement before existing vendor debt: {formatMinor((approvedSettlementDeductionState.approvedSettlement?.netPayableMinor ?? 0) - correctionPreview.difference.vendorPayableReversalMinor)}. The historical approved settlement remains unchanged. This does not create vendor debt.</p>
                                <label><span>Required Admin reason</span><textarea maxLength={500} value={currentPaidReason} onChange={(event) => setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: event.target.value })} /></label>
                                <button type="button" disabled={approvedSettlementDeductionPending || !currentPaidReason.trim()} onClick={() => void runApprovedSettlementDeduction()}>
                                  {approvedSettlementDeductionPending ? 'Applying...' : 'Approve & Apply Correction'}
                                </button>
                              </section>
                            ) : null}
                          {correctionPreview.economicDirection === 'VENDOR_CREDIT' &&
                            correctionPreview.difference.vendorPayableReversalMinor < 0 && paidCreditState?.eligible &&
                            !paidCreditApplication ? (
                              <section className="op-panel-section" aria-label="Paid correction credit application">
                                <h5>Paid payout vendor credit</h5>
                                <p className="page-description">Applying {formatMinor(-correctionPreview.difference.vendorPayableReversalMinor)} creates a standalone future vendor payable credit. The historical PAID payout remains unchanged.</p>
                                <label><span>Required Admin reason</span><textarea maxLength={500} value={currentPaidReason} onChange={(event) => setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: event.target.value })} /></label>
                                <button type="button" disabled={paidCreditPending || !currentPaidReason.trim()} onClick={() => void runPaidCredit()}>
                                  {paidCreditPending ? 'Applying...' : 'Approve & Apply Credit'}
                                </button>
                              </section>
                            ) : null}
                          {correctionPreview.economicDirection === 'VENDOR_CREDIT' &&
                            correctionPreview.difference.vendorPayableReversalMinor < 0 &&
                            beforeSettlementCreditState?.eligible && !beforeSettlementCreditApplication ? (
                              <section className="op-panel-section" aria-label="Before-settlement correction credit application">
                                <h5>Before-settlement vendor credit</h5>
                                <p className="page-description">Sporgym owes vendor {formatMinor(-correctionPreview.difference.vendorPayableReversalMinor)}. This creates a separate gross future payable source. No current settlement or payout exists for the affected finance.</p>
                                <label><span>Required Admin reason</span><textarea maxLength={500} value={currentPaidReason} onChange={(event) => setPaidCorrectionReason({ reviewId: eligibleTerminalDetail.id, value: event.target.value })} /></label>
                                <button type="button" disabled={beforeSettlementCreditPending || !currentPaidReason.trim()} onClick={() => void runBeforeSettlementCredit()}>
                                  {beforeSettlementCreditPending ? 'Applying...' : 'Approve & Apply Correction'}
                                </button>
                              </section>
                            ) : null}
                          {correctionPreview.economicDirection === 'VENDOR_CREDIT' &&
                  ...
                  {paidCreditError ? <p className="op-alert op-tone-danger" role="alert">{paidCreditError}</p> : null}
                  {paidCreditQuery.error ? <p className="op-alert op-tone-attention" role="status">Paid credit history unavailable: {paidCreditQuery.error}</p> : null}
                  {paidCreditApplication ? (
                    <MetadataGroup title="Applied financial correction credit">
                      <MetadataRow label="Status" value="Applied" />
                      <MetadataRow label="Direction" value="Vendor credit" />
                      <MetadataRow label="Gross credit" value={formatMinor(paidCreditApplication.grossCreditMinor)} />
                      <MetadataRow label="Admin actor" value={paidCreditApplication.authorizedByUserId} />
                      <MetadataRow label="Applied at" value={formatDate(paidCreditApplication.appliedAt)} />
                      <MetadataRow label="Correction authority" value={paidCreditApplication.id} />
                      <MetadataRow label="Credit source" value={paidCreditApplication.creditId} />
                      <MetadataRow label="Historical PAID payout" value={paidCreditApplication.historicalPayoutBatchId} />
                      <MetadataRow label="Historical payout paid at" value={formatDate(paidCreditApplication.historicalPayoutPaidAt)} />
                      <MetadataRow label="Settlement" value={paidCreditApplication.settlementApprovalId ?? 'Not yet settled'} />
                      <MetadataRow label="Payout" value={paidCreditApplication.payoutBatchId ?? 'Not yet paid'} />
                      <MetadataRow label="Reason" value={paidCreditApplication.reason} />
                      <p className="page-description">The historical PAID payout was not modified. This separate gross credit can enter a future settlement and payout.</p>
                    </MetadataGroup>
                  ) : null}
                  {beforeSettlementCreditError ? <p className="op-alert op-tone-danger" role="alert">{beforeSettlementCreditError}</p> : null}
                  {beforeSettlementCreditQuery.error ? <p className="op-alert op-tone-attention" role="status">Before-settlement credit history unavailable: {beforeSettlementCreditQuery.error}</p> : null}
                  {beforeSettlementCreditApplication ? (
                    <MetadataGroup title="Applied before-settlement financial correction credit">
                      <MetadataRow label="Status" value="Applied" />
                      <MetadataRow label="Direction" value="Vendor credit" />
                      <MetadataRow label="Gross credit" value={formatMinor(beforeSettlementCreditApplication.grossCreditMinor)} />
                      <MetadataRow label="Admin actor" value={beforeSettlementCreditApplication.authorizedByUserId} />
                      <MetadataRow label="Applied at" value={formatDate(beforeSettlementCreditApplication.appliedAt)} />
                      <MetadataRow label="Correction authority" value={beforeSettlementCreditApplication.id} />
                      <MetadataRow label="Credit source" value={beforeSettlementCreditApplication.creditId} />
                      <MetadataRow label="Route" value="Before settlement" />
                      <MetadataRow label="Settlement" value={beforeSettlementCreditApplication.settlementApprovalId ?? 'Not yet settled'} />
                  ...
                  {approvedSettlementDeductionError ? <p className="op-alert op-tone-danger" role="alert">{approvedSettlementDeductionError}</p> : null}
                  {approvedSettlementDeductionQuery.error ? <p className="op-alert op-tone-attention" role="status">Approved-settlement deduction history unavailable: {approvedSettlementDeductionQuery.error}</p> : null}
                  {approvedSettlementDeductionApplication ? (
                    <MetadataGroup title="Applied approved-settlement financial correction deduction">
                      <MetadataRow label="Status" value="Applied" />
                      <MetadataRow label="Direction" value="Vendor entitlement reduction" />
                      <MetadataRow label="Gross deduction" value={formatMinor(approvedSettlementDeductionApplication.grossDeductionMinor)} />
                      <MetadataRow label="Admin actor" value={approvedSettlementDeductionApplication.authorizedByUserId} />
                      <MetadataRow label="Applied at" value={formatDate(approvedSettlementDeductionApplication.appliedAt)} />
                      <MetadataRow label="Correction authority" value={approvedSettlementDeductionApplication.id} />
                      <MetadataRow label="Deduction source" value={approvedSettlementDeductionApplication.deductionId} />
                      <MetadataRow label="Historical approved settlement" value={approvedSettlementDeductionApplication.historicalApprovedSettlementId} />
                      <MetadataRow label="Reserved coverage" value={formatMinor(approvedSettlementDeductionApplication.coverageAmountMinor)} />
                      <MetadataRow label="Coverage" value={approvedSettlementDeductionApplication.coverageId} />
