...
function isZeroFinanceDisplayValue(value: string | null | undefined) {
  if (!isKnownFinanceValue(value)) {
    return false;
  }
...
export function OrderDetailPage() {
  ...
  const isResolvedVendorBlockedRefundFinanceEvidence =
    isVendorBlockedOrder &&
    order.cancelRefundReviewStatus?.trim().toLowerCase() === 'resolved' &&
    (
      (order.refundRecordCount ?? 0) > 0 ||
      order.latestOutboundRefundAttemptStatus?.trim().toLowerCase() === 'resolved' ||
  ...
  const shouldHideVendorEstimatedEarnings =
    postedRefundEvidencePresent || isResolvedVendorBlockedRefundFinanceEvidence || isTerminalRefundCompletedOrder;
  ...
  const currentRefundEvidencePresent =
    isResolvedVendorBlockedRefundFinanceEvidence ||
  ...
  const financePreviewRows = [
    ...
    state: row.value === ORDER_FINANCE_UNKNOWN_VALUE
      ...
      : isActiveVendorBlockedOrder
        ? 'Held estimate'
      : isResolvedVendorBlockedRefundFinanceEvidence && row.label === 'Refund impact'
        ? 'Recorded'
        : row.state,
  }));
  ...
  const settlementSummaryRows = [
    !isTerminalRefundCompletedOrder && isKnownFinanceValue(estimatedSettlementValue)
      ? {
          label: 'Estimated vendor payable',
          value: estimatedSettlementValue,
          state: isActiveVendorBlockedOrder ? 'Held estimate' : 'Estimated',
        }
      : null,
    currentRefundEvidencePresent && isKnownFinanceValue(refundImpactValue) && !isZeroFinanceDisplayValue(refundImpactValue)
      ? {
          label: 'Refund impact',
          value: refundImpactValue,
          state: isResolvedVendorBlockedRefundFinanceEvidence ? 'Recorded' : 'Estimated',
        }
      : null,
  ].filter((row): row is { label: string; value: string; state: string } => Boolean(row));
  const vendorFinancialSummaryRows = [
    ...
    currentRefundEvidencePresent && isKnownFinanceValue(refundImpactValue)
      ? {
          label: 'Refund Impact',
          value: refundImpactValue,
          helper: isResolvedVendorBlockedRefundFinanceEvidence ? 'Refund completed for this order.' : 'Refund activity linked to this order.',
        }
      : null,
  ...
  const financeTimelineItems = [
    financePreview || payoutCalculation || settlementTimelineRecord
      ? {
          ...
          title: isResolvedVendorBlockedRefundFinanceEvidence ? 'Refund accounting recorded' : 'Settlement preview generated',
          description: isResolvedVendorBlockedRefundFinanceEvidence
            ? 'Shopify refund evidence is recorded for finance review.'
          ...
          status: isResolvedVendorBlockedRefundFinanceEvidence ? 'Recorded' : 'Preview',
          tone: isResolvedVendorBlockedRefundFinanceEvidence ? 'success' as const : 'info' as const,
    ...
    currentRefundEvidencePresent
      ? {
          ...
          title: isResolvedVendorBlockedRefundFinanceEvidence
            ? 'Refund completed'
          ...
          status: isResolvedVendorBlockedRefundFinanceEvidence ? 'Completed' : isKnownFinanceValue(refundImpactValue) ? 'Estimated' : 'Pending',
          tone: isResolvedVendorBlockedRefundFinanceEvidence ? 'success' as const : 'warning' as const,
          href: refundFinanceRecord ? buildFinanceHref(refundFinanceRecord) : relatedReturns[0] ? `/returns/${relatedReturns[0].id}` : undefined,
        }
      : null,
    (financePreview || payoutCalculation || settlementTimelineRecord) && !isResolvedVendorBlockedRefundFinanceEvidence
      ? {
  ...
  if (hasCanonicalOperationalStory) {
    const vendorBlockedAt = isActiveVendorBlockedOrder
  ...
  if (!hasCanonicalOperationalStory && order.shipmentCreatedAt) {
    orderTimelineEvents.push({
      id: 'shipment-created',
  ...
  if (!hasCanonicalOperationalStory && (order.trackingNumber || order.trackingUrl)) {
    orderTimelineEvents.push({
      id: 'tracking-added',
  ...
  const visibleOrderCrossLinks = filterOperationalLinks(orderCrossLinks, audience);
  const singleFinanceCrossLink =
    visibleOrderCrossLinks.length === 1 && visibleOrderCrossLinks[0].id.startsWith('finance-')
      ? visibleOrderCrossLinks[0]
      : null;
        ...
        <main className="order-detail-main-column" aria-label="Order operations">
          ...
          <article className="order-detail-card-v2 order-workspace-panel order-shopify-details" aria-label="Shopify details section">
            ...
            {isInvalidShippingSnapshotAddress(order.orderSnapshot?.shippingAddress) ? (
              <div className="order-snapshot-address-warning" aria-label="Shopify shipping blocker">
                <span>Shipping warning</span>
                <strong>Shipping address is invalid or incomplete. Kargonomi shipment will be blocked.</strong>
              </div>
            ) : null}
            <details className="order-shopify-disclosure" aria-label="Shopify order snapshot">
              <summary className="order-shopify-disclosure-summary">
                <span>Show details</span>
                <small>Full-order Shopify values; not allocation-projected.</small>
              </summary>
              {order.splitSummary ? (
              ...
              <div className={`action-row ${isFulfillmentAuthoritativelyClosed ? 'order-fulfillment-complete-panel' : 'vendor-blocked-panel'}${operationalStory.resolvedByRefund ? ' order-fulfillment-terminal-panel' : ''}`}>
                ...
                {(isAdmin || canUseFulfillmentActions) && (visibleShipmentExecution || hasTrackingSync || hasShopifyFulfillmentSyncAttempt) ? (
                  <div className="tracking-summary-card order-tracking-summary-card">
                    ...
                    {shouldShowShipmentProviderSummary && !hasCanonicalOperationalStory && shipmentProviderSummary ? (
                      <details
                        id="provider-response-summary"
                        className="provider-response-summary admin-diagnostics-panel"
                        aria-label="Provider response summary"
                    ...
                    {shouldShowFailedShipmentRetryDiagnostics && !hasCanonicalOperationalStory && (!shipmentProviderSummary || !isAdmin) ? (
                      <div id="shipment-retry-diagnostics" className="shipment-recovery-actions" aria-label="Shipment retry eligibility">
                        <strong>{isAdmin ? 'Shipment recovery' : 'Shipment needs attention'}</strong>
                        <span>
                          {isAdmin
                ...
                {!hasCanonicalOperationalStory ? shippingConfigEditorPanel : null}
                {!hasCanonicalOperationalStory ? (
          ...
          >
            <div className="order-card-heading">
              <div>
                ...
                <p>
                  {isResolvedVendorBlockedRefundFinanceEvidence
                    ? 'Shopify refund processed. Refund impact is recorded.'
                    : isActiveVendorBlockedOrder
              ...
              <span className="order-preview-badge">
                {isResolvedVendorBlockedRefundFinanceEvidence ? 'Resolved' : isActiveVendorBlockedOrder ? 'Held' : 'Preview'}
              </span>
            </div>
            {isResolvedVendorBlockedRefundFinanceEvidence ? (
              <div className="finance-hold-notice finance-hold-resolved" aria-label="Refund completion status">
                ...
                <p>
                  Refund evidence is recorded separately from the allocation's operational state.
                </p>
              </div>
            ) : isActiveVendorBlockedOrder ? (
            ...
            {settlementSummaryRows.length ? (
            <div className={`order-financial-impact-grid order-finance-preview-grid${isActiveVendorBlockedOrder ? ' order-finance-preview-held' : ''}`} aria-label="Settlement summary values">
            ...
            <details className="order-settlement-disclosure" aria-label="Finance details">
              <summary className="order-settlement-disclosure-heading">
                <span>
                  <strong>Finance details</strong>
                  <small>Calculation inputs, deductions, and reconciliation context.</small>
                </span>
                <span>Details</span>
              </summary>
            ...
            {visibleFinanceTimelineItems.length ? (
              <details className="order-settlement-disclosure" aria-label="Finance timeline">
                <summary className="order-settlement-disclosure-heading">
                  <span>
                    <strong>Finance activity</strong>
                    <small>{ORDER_FINANCE_TIMELINE_HELPER_COPY}</small>
                  </span>
            ...
            {order.financeLedgerPreview ? (
              <details className="order-settlement-disclosure" aria-label="Finance ledger preview">
                <summary className="order-settlement-disclosure-heading">
                  <span>
                    <strong>Ledger preview</strong>
                    <small>Admin-only reconciliation trace; not settlement or payout truth.</small>
                  </span>
            ...
            {singleFinanceCrossLink?.href ? (
              <div className="order-settlement-actions" aria-label="Settlement actions">
                <Link className="button button-secondary button-compact" to={singleFinanceCrossLink.href}>
                  Open finance
                </Link>
              </div>
            ) : null}
          ...
          {visibleOrderCrossLinks.length === 1 && !singleFinanceCrossLink ? (
          ...
          ) : visibleOrderCrossLinks.length > 1 ? (
            <div className="order-linked-records-panel">
              <OperationalLinkCards
