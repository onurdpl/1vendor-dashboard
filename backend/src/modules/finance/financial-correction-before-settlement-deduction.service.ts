...
const appliedInclude = {
  deductionEffect: { include: { settlementLines: { include: { settlementApproval: true,
...
function serialize(record: AppliedDeduction) {
  ...
  if (record.applicationRoute !== 'BEFORE_SETTLEMENT_VENDOR_DEDUCTION' ||
  ...
  const settlementLine = deduction.settlementLines.find((line) => line.status === 'ACTIVE' &&
  ...
  return {
    ...
    route: 'BEFORE_SETTLEMENT_VENDOR_DEDUCTION' as const,
...
export async function getBeforeSettlementFinancialCorrectionDeductionState(reviewId: string, db: typeof prisma = prisma) {
  const existing = await db.financialCorrectionAuthority.findUnique({ where: { reviewId }, include: appliedInclude });
  if (existing) return existing.applicationRoute === 'BEFORE_SETTLEMENT_VENDOR_DEDUCTION'
    ? { application: serialize(existing), eligible: false, reasonCode: 'DEDUCTION_EFFECT_ALREADY_EXISTS' }
...
export async function applyBeforeSettlementFinancialCorrectionDeduction(input: {
  ...
  const matchingExisting = async () => {
    ...
    if (existing.applicationRoute === 'BEFORE_SETTLEMENT_VENDOR_DEDUCTION' &&
  ...
  // finance snapshot that silently omits this newly-authorized deduction.
  ...
  try {
    return await db.$transaction(async (tx) => {
      // Settlement drafting takes this same vendor lock before it snapshots candidate rows or deductions.
      ...
      await tx.financialCorrectionBaselineClaim.create({
        data: { acceptedEvidenceSnapshotId: preview.acceptedEvidence.id,
          consumerType: 'before_settlement_vendor_deduction', consumerId: authorityId },
      });
      await tx.financialCorrectionAuthority.create({ data: {
        ...
        applicationRoute: 'BEFORE_SETTLEMENT_VENDOR_DEDUCTION', authorizedByUserId: input.actorUserId, reason,
      } });
