...
import {
  ...
  ApprovedSettlementFinancialCorrectionCreditError,
  getApprovedSettlementFinancialCorrectionCreditState,
} from './financial-correction-approved-settlement-credit.service.js';
import {
  applyApprovedSettlementFinancialCorrectionDeduction,
  ApprovedSettlementFinancialCorrectionDeductionError,
...
import {
  ...
  ReviewPayoutFinancialCorrectionError,
  getReviewPayoutFinancialCorrectionState,
} from './financial-correction-review-payout.service.js';
import {
...
} from './financial-correction-paid-credit.service.js';
import {
...
export function registerFinanceRoutes(app: FastifyInstance, env: AppEnv) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const { id } = request.params as { id: string };
      try {
        return await cancelPayoutBatch(id);
      } catch (error) {
        if (error instanceof Error && error.message === 'REVIEW_GENERAL_CANCELLATION_FORBIDDEN') {
          return reply.code(409).send({ code: 'REVIEW_GENERAL_CANCELLATION_FORBIDDEN', message: error.message });
        }
        throw error;
      }
    },
  ...
  app.get(
    '/admin/finance/refund-reviews/:reviewId/financial-correction-paid-credit',
    { preHandler: [authMiddleware.authenticateRequest] },
    async (request, reply) => {
      if (request.authUser?.role !== 'admin') return reply.code(403).send({ message: 'Admin access required.' });
      const { reviewId } = request.params as { reviewId: string };
      return { ok: true as const, writesPerformed: false as const, ...(await getPaidFinancialCorrectionCreditState(reviewId)) };
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const { reviewId } = request.params as { reviewId: string };
        return { ok: true as const, application: await applyPaidFinancialCorrectionCredit({
          reviewId,
          previewFingerprint: readOptionalBodyString(body, 'previewFingerprint') ?? '',
          reason: readOptionalBodyString(body, 'reason') ?? '',
          actorUserId: request.authUser.id,
  ...
  app.get(
    '/admin/finance/refund-reviews/:reviewId/financial-correction-before-settlement-credit',
    { preHandler: [authMiddleware.authenticateRequest] },
    async (request, reply) => {
      if (request.authUser?.role !== 'admin') return reply.code(403).send({ message: 'Admin access required.' });
      const { reviewId } = request.params as { reviewId: string };
      return { ok: true as const, writesPerformed: false as const,
        ...(await getBeforeSettlementFinancialCorrectionCreditState(reviewId)) };
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!body || typeof body !== 'object' || Array.isArray(body) ||
            Object.keys(body).some((key) => key !== 'previewFingerprint' && key !== 'reason')) {
          throw new BeforeSettlementFinancialCorrectionError('INVALID_REQUEST', 400);
        }
        const { reviewId } = request.params as { reviewId: string };
        return { ok: true as const, application: await applyBeforeSettlementFinancialCorrectionCredit({
          reviewId,
          previewFingerprint: readOptionalBodyString(body, 'previewFingerprint') ?? '',
          reason: readOptionalBodyString(body, 'reason') ?? '',
          actorUserId: request.authUser.id,
  ...
  app.get(
    '/admin/finance/refund-reviews/:reviewId/financial-correction-approved-settlement-credit',
    { preHandler: [authMiddleware.authenticateRequest] },
    async (request, reply) => {
      if (request.authUser?.role !== 'admin') return reply.code(403).send({ message: 'Admin access required.' });
      const { reviewId } = request.params as { reviewId: string };
      return { ok: true as const, writesPerformed: false as const,
        ...(await getApprovedSettlementFinancialCorrectionCreditState(reviewId)) };
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!body || typeof body !== 'object' || Array.isArray(body) ||
            Object.keys(body).some((key) => key !== 'previewFingerprint' && key !== 'reason')) {
          throw new ApprovedSettlementFinancialCorrectionCreditError('INVALID_REQUEST', 400);
        }
        const { reviewId } = request.params as { reviewId: string };
        return { ok: true as const, application: await applyApprovedSettlementFinancialCorrectionCredit({
          reviewId,
          previewFingerprint: readOptionalBodyString(body, 'previewFingerprint') ?? '',
          reason: readOptionalBodyString(body, 'reason') ?? '',
          actorUserId: request.authUser.id,
  ...
  app.get(
    '/admin/finance/refund-reviews/:reviewId/financial-correction-approved-settlement-deduction',
    { preHandler: [authMiddleware.authenticateRequest] },
    async (request, reply) => {
      if (request.authUser?.role !== 'admin') return reply.code(403).send({ message: 'Admin access required.' });
      const { reviewId } = request.params as { reviewId: string };
      return { ok: true as const, writesPerformed: false as const,
        ...(await getApprovedSettlementFinancialCorrectionDeductionState(reviewId)) };
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!body || typeof body !== 'object' || Array.isArray(body) ||
            Object.keys(body).some((key) => key !== 'previewFingerprint' && key !== 'reason')) {
          throw new ApprovedSettlementFinancialCorrectionDeductionError('INVALID_REQUEST', 400);
        }
        const { reviewId } = request.params as { reviewId: string };
        return { ok: true as const, application: await applyApprovedSettlementFinancialCorrectionDeduction({
          reviewId,
          previewFingerprint: readOptionalBodyString(body, 'previewFingerprint') ?? '',
          reason: readOptionalBodyString(body, 'reason') ?? '',
          actorUserId: request.authUser.id,
  ...
  app.get(
    '/admin/finance/refund-reviews/:reviewId/financial-correction-review-payout',
    { preHandler: [authMiddleware.authenticateRequest] },
    async (request, reply) => {
      if (request.authUser?.role !== 'admin') return reply.code(403).send({ message: 'Admin access required.' });
      const { reviewId } = request.params as { reviewId: string };
      return { ok: true as const, writesPerformed: false as const,
        ...(await getReviewPayoutFinancialCorrectionState(reviewId)) };
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const body = request.body;
        if (!body || typeof body !== 'object' || Array.isArray(body) ||
            Object.keys(body).some((key) => key !== 'previewFingerprint' && key !== 'reason' && key !== 'confirmEftNotSent')) {
          throw new ReviewPayoutFinancialCorrectionError('INVALID_REQUEST', 400);
        }
        const { reviewId } = request.params as { reviewId: string };
        return { ok: true as const, application: await applyReviewPayoutFinancialCorrection({
          reviewId,
          previewFingerprint: readOptionalBodyString(body, 'previewFingerprint') ?? '',
          reason: readOptionalBodyString(body, 'reason') ?? '',
          confirmEftNotSent: (body as { confirmEftNotSent?: unknown }).confirmEftNotSent === true,
          actorUserId: request.authUser.id,
        }) };
      } catch (error) {
