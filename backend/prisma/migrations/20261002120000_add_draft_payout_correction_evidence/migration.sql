ALTER TABLE "FinancialCorrectionAuthority"
  ADD COLUMN "historicalDraftPayoutGrossMinor" INTEGER,
  ADD COLUMN "historicalDraftPayoutNetMinor" INTEGER,
  ADD COLUMN "historicalDraftPayoutDebtOffsetMinor" INTEGER,
  ADD COLUMN "historicalDraftPayoutSourceFingerprint" TEXT,
  ADD COLUMN "historicalDraftPayoutCancelledAt" TIMESTAMP(3);
...
BEGIN
  ...
  IF direction IS DISTINCT FROM 'VENDOR_CREDIT' OR
     NOT ((route = 'PAID_VENDOR_CREDIT' AND payout_id IS NOT NULL AND payout_paid_at IS NOT NULL AND approval_id IS NULL) OR
          ...
          (route = 'DRAFT_PAYOUT_VENDOR_CREDIT' AND payout_id IS NOT NULL AND payout_paid_at IS NULL AND approval_id IS NOT NULL)) OR
...
BEGIN
  SELECT "applicationRoute", "economicDirection", "vendorId", "currency", "vendorPayableDifferenceMinor"
    ...
    FROM "FinancialCorrectionAuthority" WHERE "id" = NEW."authorityId";
  IF authority_route NOT IN ('BEFORE_SETTLEMENT_VENDOR_DEDUCTION', 'APPROVED_SETTLEMENT_VENDOR_DEDUCTION', 'DRAFT_PAYOUT_VENDOR_DEDUCTION') OR
...
BEGIN
  ...
  IF source_route NOT IN ('APPROVED_SETTLEMENT_VENDOR_DEDUCTION', 'DRAFT_PAYOUT_VENDOR_DEDUCTION') OR
